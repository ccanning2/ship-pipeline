...
Two label groups track the parent. Each group allows one label at a time, so the labels always show where the ticket is and who holds it.
...
## Child tickets
Child tickets are always created as sub-issues of the parent.
...
State mapping to Linear workflow states: open/reopened → Todo, in-progress → In Progress, fixed → In Review, verified/done → Done, wontfix → Canceled.
