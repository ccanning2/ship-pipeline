...
| SHI-21 | defect | qa | Medium | open | engineer | init.sh: a flagless re-run recreates scripts/deploy/* and deploy.yml in a project whose pipeline.env declares no deployable environments |
| SHI-22 | defect | qa | Low | open | engineer | init.sh: an unknown --profile is rejected only after tooling files have been copied (half-applied install) |
| SHI-23 | defect | qa | Low | open | engineer | commands/pipeline-init.md names a vendor (Hetzner); AC-31 / NFR-10 require none in agents/ or commands/ |
